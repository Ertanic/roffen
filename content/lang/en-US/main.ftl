...
main-page-template-function-11 = returns all components.
main-page-template-function-12 = renders a component.
main-page-template-function-13 = returns a translation by fluent key.
main-page-template-value-3 = route parameters.
...
main-page-components-implementation-paragraph-2 = Therefore, I suggest that you first look at the component description syntax. The component description is written in KDL.
main-page-components-required = Required
main-page-components-required-paragraph-1 = Next, you must describe the HTML structure of the component. You can use the <code>html</code> keyword to describe the HTML structure. The next component will generate a <code>div</code> with specific classes.
main-page-components-required-paragraph-2 = A component can have child components, which can be described using the <code>children</code> statement. Let's describe a list item. To prevent it from being empty, use the <code>content</code> statement inside the <code>children</code> section.
main-page-components-required-paragraph-3 = Using this <code>#name</code> or this <code>#(name)</code> syntax, you can integrate property values directly into your HTML code. As an example, you can use this syntax directly in HTML tags, changing them on the fly, as is done with <code>h#number</code>, which will result in <code>h1</code>.
main-page-components-properties = In general, the component can already be used, but if you need to set a value manually, you can use properties. Properties are quite useful on their own, as they allow you to implement reactive interaction with HTML code without additional lines of JS code, which can be useful in some cases.
main-page-components-optional = Optional
main-page-components-optional-paragraph = Optionally, you can specify additional properties for the component container. Yes, the component's HTML code is not directly generated in another part of the markup; it is generated within the <code>div.grid-block</code>. To interact with this block, you can use the <code>container</code> statement and reassign some default values.
main-page-components-rendering-step-2 = Get a list of all the components.
main-page-components-rendering-step-3 = Iterate over the post content.
main-page-components-rendering-step-4 = Call a special function to generate HTML code.
main-page-components-rendering-step-5 = To view the results of the script, see the Posts page.
